...
lib.nixvim.neovim-plugin.mkNeovimPlugin {
  ...
  extraOptions = {
    colorscheme =
      ...
      defaultNullOpts.mkNullable' {
        ...
        pluginDefault = lib.literalMD ''`vim.env.base46_THEME` or `"schemer-dark"`'';
        description = ''
          The base46 colorscheme to use.
          ...
          `:h nvim-base46-builtin-colorschemes` includes a full list of builtin themes,
          however the [plugin's source code] may be more up to date.

          You can access `require('${luaName}')` as `base46` in any raw lua,
          for example, you could reuse some colors from the builtin colorschemes:

          ```nix
            base03.__raw = "base46.colorschemes['catppuccin'].base06";
          ```

          [plugin's source code]: https://github.com/RRethy/base46-nvim/blob/master/lua/colors/init.lua
        '';
    ...
    setUpBar = lib.mkOption {
      ...
      default = true;
      example = false;
      description = "Whether to set your status bar theme to 'base46'.";
    };
  };
  ...
  extraConfig = cfg: {
    ...
    # See https://github.com/RRethy/base46-nvim/blob/6ac181b5733518040a33017dde654059cd771b7c/lua/base46-colorscheme.lua#L107-L125
    colorschemes.base46.luaConfig.content = ''
      do
        local base46 = require('${luaName}')
        base46.with_config(${toLuaObject cfg.settings})
        base46.setup(${toLuaObject cfg.colorscheme})
      end
    '';
  };
}
